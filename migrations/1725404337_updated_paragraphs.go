...
func init() {
	m.Register(func(db dbx.Builder) error {
		...
		collection.ListRule = types.Pointer("@request.auth.id != \"\" && (@request.auth.role ~ \"admin\" || @request.auth.id = user)")

		collection.ViewRule = types.Pointer("@request.auth.id != \"\" && (@request.auth.role ~ \"admin\" || @request.auth.id = user)")
